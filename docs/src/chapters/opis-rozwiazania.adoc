...
Z tego powodu funkcjonalność otrzymała swój własny pakiet.
...
_handleNotificationResponse_ oraz _handleNotificationDenial_ klasy _NotificationService.
...
która sprawdza, czy wobec powiadomienia została podjęta już jakaś akcja. Następnie pobierana jest klasa obsługująca
...
wzorzec projektowy strategia (_strategy_)<<gof>>. Dzięki temu każdy tym powiadomienia ma swoją własną klasę
odpowiedzialną za jego obsługę.
...
_NotificationHandlerFactory_. Jest to implementacja wzorca projektowego fabryka (_factory_)<<gof>>. Pozwala ona
na pobranie implementacji na podstawie wartości typu wyliczeniowego _NotificationType_.
...
zaimplementowaną metodę _confirm_.
...
Następnie wywoływana jest prywatna metoda _updateRawTransaction_, odpowiedzialna za aktualizację transakcji.
...
zajmuje się już przesłaniem transakcji do innych węzłów w sieci Bitcoin.
...
potwierdzaniem transakcji można ją również odrzucić. Wykonuje to metoda _deny_ klasy _TransactionHandler_.
...
nieosiągalny, to oczekujący portfel zostanie usunięty. W przypadku metody _confirm_ logika prezentuje się następująco:
...
jest ona przechowywana w katalogu domowym użytkownika i może być modyfikowana z poziomu komputera, bądź aplikacji
webowej. Plik ten kopiowany jest do docelowego katalogu, gdy nie znajduje się tam już istniejąca konfiguracja.
