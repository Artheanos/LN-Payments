...
.Fragment kodu źródłowego rozdziału "analiza".
[source, asciidoc]
----
...
Są to między innymi różnego rodzaju wykresy i rzuty ekranu. Ostatnim folderem zawierającym kod źródłowy dokumentacji
...
Ostatnim folderem w repozytorium projektu jest _devtools_. Zawiera on dodatkowi pliki związane z programami
wykorzystywanymi w pracach nad projektem. Aktualnie zawiera on foldery dla programów _postman_ oraz _docker_.
...
systemy, czyli węzeł sieci Lightning, węzeł sieci Bitcoin oraz bazę danych. Dostępne są trzy pliki _docker-compose_.
Jeden z nich jest plikiem bazowym, zawierającym podstawową konfigurację i nie powinno się go uruchamiać samemu.
Pozostałe dwa pliki odpowiadają dwóm aktualnie wspieranym sieciom Bitcoina, czyli _regtest_ oraz _testnet_. Katalog
ten zawiera również konfigurację dla obu węzłów. Dostępne są one odpowiednio w folderach _bitcoind_ oraz _lnd_. Są
one przesyłane do kontenera podczas jego rozruchu i zawierają ustawienia dopasowane do tworzonego systemu.
