...
W kodzie 159 zaprezentowano przykładowy test jednostkowy. Oczekuje się, że po prawidłowym dodaniu administratora
...
----
Kolejny fragment kodu, pokazany w kodzie 160, zawiera natomiast test integracyjny. Służy on do sprawdzenia, czy w
...
Fragment kodu umieszczony w kodzie 161 przedstawia test odpowiedzialny za sprawdzanie, czy komponent _navbar_ zostanie
...
Kolejny test zaprezentowany w kodzie 162 odpowiedzialny jest za sprawdzenie formularza zmiany hasła. Po wprowadzeniu
...
Przykładowy test odpowiedzialny za generowanie komponentu zaprezentowano w kodzie 163.
...
oraz języka _Gherkin_. Przykładowy scenariusz został zamieszczony w kodzie 164.
...
zobaczyć na załączonym kodzie 165.
