...
System w kontekście kryptowalut podzielić można na cztery części. Środki będą sekwencyjnie przychodzić przez wszystkie
z nim. Cykl życia środków w systemie zacznie się od sieci Lightning. Użytkownik będzie musiał utworzyć kanał płatności
z tworzonym serwerem, dzięki czemu będzie mógł zakupić tokeny. Środki ze sprzedaży początkowo zablokowane będą w
kanale płatności, które będą musiały być regularnie zamykane, aby móc wykorzystać zebrane bitcoiny. Następnie fundusze
będą musiały zostać przelane na adres multisig, który utworzony będzie z kilku kluczy publicznych osób zarządzających
serwerem. Cały proces kończy się na transakcjach, które pozwalają na wykorzystanie środków zgromadzonych w portfelu
poprzez mechanizm podpisywania transakcji w aplikacji mobilnej. Poniżej przedstawiony został szczegółowy opis każdej z
czterech części.
...
oraz opłat transakcyjnych zostało już opisanych w podrozdziale <<_przedstawienie_problemu,"przedstawienie problemu">>.

W kontekście tej pracy kupowanie tokenów będzie odbywało się tylko za pomocą sieci Lightning. Z założenia cena różnego
rodzaju tokenów jest tak niska, że górnicy i tak by nie chciały wykopać takiej transakcji. Problematyczne
...
skrypcie odblokowującym. Skrypt odblokowujący zawiera natomiast podpisy cyfrowe oraz cały _redeem script_.
...
osób. Nic nie stoi na przeszkodzie nieuczciwej osobie posiadającej dostęp do kradzieży bądź nieuczciwego
rozdysponowania środków. Ten sam problem dotyczy sklepów internetowych, zarówno przyjmujących płatności w
...
z sejfem. Każda uprawniona osoba może samodzielnie rozdysponować środki bez wiedzy współpracowników.

Adres multisig w proponowanym rozwiązaniu ma na celu rozwiązanie tego problemu. Rozdysponowanie środków wymaga
podpisów kluczami prywatnymi określonej liczby administratorów serwera. Dzięki temu jedna uprawniona osoba nie ma
możliwości samodzielnego decydowania o dyspozycji środków. Musi to być zawsze decyzja kolektywna. Dodatkowym
zabezpieczeniem jest aplikacja mobilna do podpisywania transakcji, która przechowuje jednocześnie klucze prywatne.
Dzięki temu proces podpisywania nie odbywa się w jednym centralnym miejscu, co znacząco zmniejszałoby bezpieczeństwo.
Atak hakerski na serwer mógłby spowodować utratę środków. Haker do rozdysponowania środków znajdujących się na
serwerze będzie musiał poza serwerem uzyskać dostęp do kilkunastu urządzeń mobilnych przechowujących klucze.
Atak na tak wiele urządzeń jest niezwykle trudny, prawie niewykonalny bez posiadania odpowiedniej wiedzy o lokalizacji
urządzeń. Proponowane rozwiązanie sprawia, że środki znajdujące się na serwerze są niezwykle bezpieczne, a
prawdopodobieństwo utraty środków w niekontrolowany sposób jest zerowe.
...
zabezpieczony na możliwość utraty, bądź uszkodzenia urządzenia zawierającego klucze. Dzięki _mnemonic phrase_ będzie
