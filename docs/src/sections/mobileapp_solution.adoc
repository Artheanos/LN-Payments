...
jest plik _App.tsx_. Zawiera on część konfiguracji oraz jest to główny komponent aplikacji grupujący kontekst
użytkownika i pierwszy element nawigacji.
...
wartości zapisanych lokalnie w pamięci.
...
_DrawerRouter_, który odpowiedzialny jest za menu boczne aplikacji.
