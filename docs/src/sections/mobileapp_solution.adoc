...
Aplikacja w swojej pamięci zapisuje adres email, adres URL serwera, token JWT oraz nazwę kanały WebSocket, na który
...
Główna część aplikacji mobilnej znajduje się w folderze _components_. Zawiera on kod wszystkich ekranów komponentów
oraz nawigacji. Pliki mają rozszerzenie _tsx_, ponieważ zapisane są w formacie narzędzia _Babel_, które pozwala
...
jest plik _App.tsx_. Zawiera on część konfiguracji aplikacji główny komponent grupujący kontekst użytkownika i
pierwszy element nawigacji.
...
_UserContext.tsx_ w folderze _context_.
...
lokalnie w urządzeniu mobilnym. Kontekst zawiera również funkcje pozwalające na manipulowanie obiektem użytkownikiem,
...
Jego prawdziwe właściwości definiowane są w komponencie _UserContextProvider_.
...
wartości zapisanych lokalnie na dysku.
...
z pamięci, niezależnie od pozostałych pól użytkownika.
...
_logoutUser_, która pozwala na wylogowanie użytkownika.
...
Który jest punktem wejściowym do nawigacji w aplikacji mobilnej.
...
do ekranu ze szczegółami powiadomienia, a jako parametr podane jest jego identyfikator. Następnie funkcja nasłuchująca
...
użytkownik wypełni formularz i go wyśle, zostanie wywołana metoda _onSubmit_:
...
wyświetlenie odpowiednich komunikatów. Funkcja _onSuccess_ wywoływana jest w przypadku powodzenia logowania.
Aktualizuje ona kontekst użytkownika i lokalny magazyn danych, ustawiając dane otrzymane w odpowiedzi żądania.
...
informującą o aktualnie trwającym etapie procesu. Główna logika znajduje się natomiast w funkcji _generateKeys_.
Funkcja wywoływana jest przez hak _useEffect_ przy tworzeniu komponentu.
...
Można ną funkcję podzielić na 3 części, czyli generowanie kluczy, przesyłanie klucza publicznego na serwer oraz
...
jak również kod wysyłający powiadomienia push.
...
jest _showNotification_, która odpowiedzialna jest za wyświetlanie powiadomień push.
...
_DrawerRouter_, który odpowiedzialny jest za manu boczne aplikacji.
...
Definiuje samo menu oraz jego zawartość. Generowanie całej części widoku użytkownika odbywa się automatycznie.
...
jest w komponencie NotificationListScreen.
...
do wyświetlenia. Elementy powiadomień pobierane są przy inicjacji komponentu przez funkcję _queryElements_.
...
Pobieranie kolejnej strony odbywa się przy pomocy metody _loadElements_. Jeżeli wartość flagi _hasMore_ jest prawdziwa,
to wywoływana jest funkcja _loadElements_ z wartością strony większą o jeden. Komponent ten zawiera jeszcze metodę
_onRefresh_. Jest ona wywoływana przez komponent _FlatList_, gdy użytkownik wykona pociągnięcie listy w dół.
Uruchamia to odświeżenie listy.
...
Drugi przycisk pozwala na odrzucenie powiadomienia, co wykonywane jest przez funkcję _deny_.
