...
gdyż odbywa się do automatycznie przez używane szablon aplikacji. Wbrew pozorom, było to łatwiejsze, niż stworzenie
...
o wartości 22 punktów zostało przeniesionych do następnej iteracji. Jest to bardzo zły wynik, który tylko potęguje
opóźnienie w realizacji projektu. Nierówny wkład własny członków zespołu powoduje, że zadania związane z serwerem
kończone są zawsze na czas, natomiast zadania związane z _frontend'em_ stale nie są dostarczane w terminie.
