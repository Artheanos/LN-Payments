...
Priorytetem tej iteracji był rozwój aplikacji mobilnej. Na początku praz napotkano kilka poważnych problemów.
...
posunięcie to przyspieszyło prace nad komponentami systemu.

Po uporaniu się ze środowiskiem aplikacji mobilnej przystąpiono do jej implementacji. Jako pierwsze zostało zrealizowane
zadanie LP-149. Miało ono początkowo dodać nowy ekran przed logowaniem, w którym użytkownik musiałby podać adres
...
Dwa następne zadania dotyczą dalszych prac nad serwerem. Komponent ten był już bliski ukończenia, a zadań w rejestrze
...
wznowić prace nad aplikacją mobilną. Problemu te są częściowym usprawiedliwieniem w niepowodzeniach tej iteracji, gdyż
