...
Oprócz języka _JavaScript_ zespół korzysta z jego nadzbioru o nazwie — Typescript.
Pozwala on na wykrycie wielu błędów jeszcze przed uruchomieniem kodu.
Dodatkowo stanowczo usprawnia pracę dzięki temu, że edytory tekstowe, z których korzysta zespół, wyświetlają typ i pola
obiektu zaznaczonego kursorem.
...
Zespół zdecydował się na używanie komponentów funkcyjnych, ponieważ są one bardziej przejrzyste, zużywają mniej kodu i
...
Z katalogu _pages_ są udostępniane komponenty stanowiące główną część lub cały widok pojedynczej strony.
...
* _quickBuy_ — przyjmowanie pieniędzy i zwracanie tokenów
* _userPanel_ — widoki dla zalogowanych użytkowników
** _account_ — edycja swojego konta
** _adminManagement_ — zarządzanie kontami adminów
** _serverSettings_ — zarządzanie konfiguracjami serwera
** _transactions_ — zarządzanie transakcjami
** _wallet_ — zarządzanie portfelem

Katalog _components_ zawiera komponenty, które nie znajdują się w katalogu _pages_.
Są wyodrębnione w taki sposób, aby mogły być wykorzystane w dowolnym miejscu w aplikacji.
Pozwala to na ponowne użycie komponentów. Na przykład komponent do przyjmowania płatności _QuickBuy.tsx_,
który można zobaczyć w panelu użytkownika i
...
* _Context_ — komponenty zwracające _Reactowy context_ z przygotowanymi danymi i hookami
* _Form_ — komponenty do tworzenia formularzy
* _Layouts_ — komponenty, które wyświetlają inne komponenty dodając do nich pewne elementy
...
* jest.config.js — zawiera konfiguracje takich rzeczy jak generowanie raportów z testów,
...
że token jest w trakcie ładowania.
...
* _UserLayout.tsx_ — przekierowuje do strony logowania, jeśli użytkownik nie jest zalogowany, lub do strony głównej,
jeśli użytkownik ma rolę tymczasową
* _AdminLayout.tsx_ — przekierowuje do strony logowania, jeśli użytkownik nie jest zalogowany, lub do strony głównej,
jeśli użytkownik nie jest adminem
...
do serwera. Jeśli odświeżanie tokena się nie powiedzie, użytkownik jest wylogowany, a zarazem przekierowany
przez _UserLayout.tsx_ lub _AdminLayout.tsx_ do strony logowania.
Jeśli odświeżanie się powiedzie, kolejne jego uruchomienie zostaje zakolejkowane do wykonania za 10 minut.
Ponieważ ważność tokena na serwerze jest ustalona na 15 minut, token w przeglądarce jest zawsze ważny.
...
Context _UserContext.tsx_ obejmuje wszystkie widoki,
tak żeby dane o użytkowniku nie zostały usunięte przy zmianie widoków.
...
API _localStorage_ pozwala zapisać dane typu klucz wartość, ale klucz i wartość muszą być wartością tekstową.
...
Funkcja _setLocalJson_ zamienia przyjętą wartość w ciąg tekstowy i zapisuje go w _localStorage_.
...
Funkcja _getLocalJson_ pobiera wartość z _localStorage_ i zamienia ją z ciągu tekstowego na obiekt _JSON_,
przy okazji zamieniając ciągi tekstowe na obiekty _Date_.
...
Ponieważ w Reakcie zmienne wartości muszą być przechowywane w stanie, został napisany hook _useLocalStorage_,
...
który wskazuje na wartość w _localStorage_.
...
Każdy przycisk na tym pasku jest zdefiniowany przy pomocy komponentu _SidebarEntry.tsx_.
...
Tekst jest zapisany w postaci plików _json_ w folderze _src/locales_ pogrupowanych na domeny.
...
która dodaje do zapytania nagłówek z tokenami i wykonuje je przy użyciu biblioteki _axios_.
...
Ponieważ serwer zwraca różne kody HTTP, a biblioteka _axios_ wyrzuca błąd,
jeśli kod odpowiedzi nie jest kodem powodzenia,
metoda _request_ wyłapuje błąd zwracany przez axios i zwraca obiekt typu _Response_ z pustym polem danych,
...
Widok portfelu składa się z kafelek, dlatego powstał komponent _WalletCard.tsx_,
...
Przyjmuje dwie wartości liczbowe i zapełnia pasek ładowania zgodnie ze stosunkiem tych dwóch liczb.
...
Gdy dane o portfelu są w trakcie pobierania, widok profelu wyświetla _WalletLoadingSkeleton.tsx_.
...
Jeśli na widoku portfelu serwer nie ma stworzonego portfelu,
zostanie wyświetlony _WalletCreatePage.tsx_.
Żeby stworzyć portfel, potrzebny jest co najmniej jeden admin z przypisanym kluczem publicznym.
W komponencie _WalletCreatePage.tsx_ pobierani są użytkownicy z rolą _admin_
i filtrowani sprawdzając obecność klucza publicznego.
Jeśli długość tablicy zawierającej filtrowanych adminów wynosi zero,
zamiast formularza zwracany jest komunikat o błędzie.
...
.Komunikat o błędzie w widoku formularza portfelu.
image::../images/system_presentation/webapp/wallet_cannot_create.png[]

==== Płatność

Za przyjmowanie płatności odpowiedzialny jest komponent _QuickBuy.tsx_.
...
funkcje do przechodzenia do następnego komponentu.
...
Znajduje się on w pliku _SetupStage.tsx_.
...
używając telefonu lub kopiując do schowka poprzez naciśnięcie na ten kod.
...
jeśli komponent zostanie zamknięty.
...
Ostatnim krokiem jest wyświetlenie tokenów zwróconych przez serwer poprzez komponent _TokensStage.tsx_.
...
że tokeny nie będą mogły być odczytane po zamknięciu widoku.
...
Ten widok wykorzystuje plik _Panel_, który składa się z trzech komponentów,
...
Ponieważ w większości widoków z tabelami zespół chce pobrać listę danych z danej ścieżki i ją wyświetlić,
...
Dlatego powstał komponent _PageableTable.tsx_, wykorzystywany przez _ApiPageableTable.tsx_,
który przyjmuje listę danych i wyświetla ją w postaci tabeli.
...
Każdy komponent zagnieżdżony w tym context'ście ma możliwość wyświetlenia powiadomienia, która nie zniknie przy
zamknięci komponentu, który wyświetlił powiadomienie.
...
Do zmieniania stanu powiadomienia wykorzystywany jest hook _useNotification_.
...
Wygląd powiadomienia został zaimplementowany w pliku _src/components/common/Notification.tsx_.
...
_ConfirmationModal.tsx_ to okno dialogowe, wyświetla przekazany tekst z dwoma przyciskami —
do odrzucenia i do zaakceptowania wyświetlonej informacji.
Jeśli wiadomość zostanie odrzucona, okno zostaje zamknięte.
Jeśli wiadomość zostanie zaakceptowana, okno zostaje zamknięte,
