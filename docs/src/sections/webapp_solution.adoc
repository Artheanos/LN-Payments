...
** _wallet_ — tworzenie i wyświetlanie statystyk portfelu

Katalog _components_ zawiera komponenty, które nie zostały zamieszczone w katalogu _pages_.
Zostały wyodrębnione w taki sposób, aby mogły być wykorzystane w dowolnym miejscu w aplikacji.
...
* _Context_ — komponenty zwracające _React_ context z przygotowanymi danymi i hookami
...
do serwera. Jeśli odświeżanie tokena się nie powiedzie, użytkownik zostanie wylogowany, a zarazem przekierowany
...
Dzięki temu, że ważność tokena na serwerze jest ustalona na 15 minut, token w przeglądarce jest zawsze ważny.
...
API _localStorage_ pozwala zapisać dane typu klucz wartość, ale tylko postaci wartości tekstowej.
...
Z tego powodu, że w Reakcie zmienne wartości muszą być przechowywane w stanie, został napisany hook _useLocalStorage_,
...
Istnieje możliwość przekazania do niego argumentu _adminOnly_,
jeśli przycisk ma być wyświetlony tylko dla administratorów.
...
Serwer używa różnych kodów HTTP do przekazania różnych informacji, w tym kodów błędu.
...
Przy wyświetlaniu stanu środków portfelu i środków w kanałach _Lightning Network_.
