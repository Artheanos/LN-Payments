...
nieautoryzowanego użytkownika. Następnie wywoływana jest metoda _buildInfoResponse_ klasy _PaymentFacade_. Przyrostek
...
findAll, które delegowane jest aż do repozytorium w niezmienionej formie, co kończy działanie tego procesu.
...
Implementuje ona interfejs _Specification_, który pozwala na ręczne tworzenie kwerend do bazy danych
