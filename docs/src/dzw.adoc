...
wydarzeń w zewnętrznym systemie. Serwer po wygenerowaniu tokenu będzie wysyłał go na serwer klienta, rejestrując
...
posiadali konta, ale będą mieli taką możliwość. Bez logowania będą mieli dostęp tylko do podstrony pozwalającej na
wykonanie płatności w sieci lightning, na której będzie wyświetlany token który zakupił. System będzie posiadał
wbudowany portfel, na który przelewane będą środki. Aby wykonać przelew z wbudowanego portfela, wymagane będą
...
* Możliwość przeglądania tokenów (które zostały wygenerowane i wysłane do Użytkownika po dokonaniu przez niego płatności)
...
== Wizja konstrukcyjna
W części back-endowej skorzystamy z Javy 11, Springa, Bitcoinj i LightningJ. Do testów użyjemy JUnit 5, Mockito oraz REST-assured.
...
*Token* - ciąg znaków uprawniający do podpisywania zdażeń (albo jakikolwiek inny kod) +
